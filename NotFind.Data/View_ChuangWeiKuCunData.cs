...
{
    public class View_ChuangWeiKuCunData
    {
        ...
        {
            DataSet _DataSet = SqlHelper.ExecuteDataSet(SqlHelper.ConnectionString2, CommandType.StoredProcedure, "View_ChuangWeiKuCun_List", null);
        ...
        public static Hashtable List4Page(String _RuZhuStartDate, String _TuiFangEndDate, bool _ShiFouShanChu, Int32 _BaoLiuZhuangTai, Int32 _XiTongSN, Int32 _BingGuanSN, Int32 _FangXingSN, String _sql, Int32 _RecordCount, Int32 _PageIndex)
        {
            ...
            if (_XiTongSN != 0)
            {
                Sql += " and C_SuoShuXiTongSN = " + _XiTongSN;
            }

            //拼接宾馆SN
            if (_BingGuanSN != 0)
            {
                Sql += " and C_BingGuanSN = " + _BingGuanSN;
            }
            //拼接房型SN
            if (_FangXingSN != 0)
            {
                Sql += " and C_FangXingSN = " + _FangXingSN;
            }

            //凭借是否删除
            if (_ShiFouShanChu)
            {
                Sql += " and C_ShiFouShanChu =" + 1;
            }
            else
            {
                Sql += " and C_ShiFouShanChu =" + 0;
            }
            ...
            if (_RuZhuStartDate != "")
            {
                Sql += " and C_RuZhuStartDate <='" + _RuZhuStartDate + " 12:00:00'";
            }
            //最晚退房时间
            if (_TuiFangEndDate != "")
            {
                Sql += " and C_TuiFangEndDate >='" + _TuiFangEndDate + " 12:00:00'";
            }

            if (_BaoLiuZhuangTai != 0)
            {
                Sql += " and C_BaoLiuZhuangTai =" + _BaoLiuZhuangTai;
            }
            ...
            int RecordCount = Convert.ToInt32(SqlHelper.ExecuteScalar(SqlHelper.ConnectionString2, CommandType.Text, "select count(C_SN) from View_ChuangWeiKuCun where " + Sql, null).ToString());//记录总数
            ...
            SqlParameter[] parm = {
                                      new SqlParameter("TableName", "View_ChuangWeiKuCun"), //表名、视图名
                                      new SqlParameter("FdShow", "C_BingGuanSN,B_Name,C_SuoShuXiTongSN,D_Name,C_FangXingSN,F_FangJianHao,F_Name,C_SN,C_SuoShuBuMenSN,C_FangJianSN,C_RuZhuStartDate,C_TuiFangEndDate,C_BaoLiuZhuangTai,C_ShiFouShanChu,C_ChuangWei1JiaGe,C_ChuangWeiHao"), //要显示的字段列表
                                      new SqlParameter("Critical", Sql), //条件语句
                                      new SqlParameter("FdOrder", "C_SN desc"), //排序字段列表
                                      new SqlParameter("PageSize", _RecordCount), //每页的大小(行数)
                                      new SqlParameter("PageCurrent", _PageIndex), //要显示的页
                                      new SqlParameter("SN", "C_SN"), //ID键名称，如R_SN
                                  };
